...
function findDirectivesWithMissingMixedContentPropertyFromHelmet(context, node) {
    ...
    if (args.length === 1) {
        const [options] = args;
        const maybeDirectives = (0, utils_1.getObjectExpressionProperty)(options, DIRECTIVES);
        if (maybeDirectives &&
...
function isValidHelmetModuleCall(context, callExpr) {
    ...
    /* csp(options) */
    if (callee.type === 'Identifier' && ((_a = (0, utils_1.getModuleNameOfNode)(context, callee)) === null || _a === void 0 ? void 0 : _a.value) === HELMET_CSP) {
        return true;
    }
    /* helmet.contentSecurityPolicy(options) */
    return (0, utils_1.isCallToFQN)(context, callExpr, HELMET, CONTENT_SECURITY_POLICY);
}
function isMissingMixedContentProperty(directives) {
    return !(Boolean((0, utils_1.getObjectExpressionProperty)(directives.value, BLOCK_ALL_MIXED_CONTENT_CAMEL)) ||
        Boolean((0, utils_1.getObjectExpressionProperty)(directives.value, BLOCK_ALL_MIXED_CONTENT_HYPHEN)));
}
