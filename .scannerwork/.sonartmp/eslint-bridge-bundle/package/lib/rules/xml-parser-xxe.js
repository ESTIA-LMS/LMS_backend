...
exports.rule = {
    ...
    create(context) {
        ...
        function isXmlLibraryImported() {
            return (0, utils_2.getImportDeclarations)(context).findIndex(i => i.source.value === XML_LIBRARY) > -1;
        }
        function isXmlLibraryRequired() {
            return ((0, utils_2.getRequireCalls)(context).findIndex(r => r.arguments[0].type === 'Literal' && r.arguments[0].value === XML_LIBRARY) > -1);
        }
        ...
        return {
            CallExpression: (node) => {
                ...
                if (isXmlParserCall(call) && isXmlLibraryInScope()) {
                    const noent = (0, utils_2.getObjectExpressionProperty)(call.arguments[1], 'noent');
                    if (noent && isNoEntSet(noent)) {
                        context.report({
                            message: (0, utils_1.toEncodedMessage)('Disable access to external entities in XML parsing.', [
                                call.callee,
                            ]),
                            node: noent,
                        });
