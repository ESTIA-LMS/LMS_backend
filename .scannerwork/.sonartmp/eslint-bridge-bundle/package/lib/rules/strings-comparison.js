        ...
        return {
            BinaryExpression: (node) => {
                ...
                if (['<', '<=', '>', '>='].includes(operator) &&
                    (0, utils_1.isString)(left, services) &&
                    (0, utils_1.isString)(right, services) &&
                    !isLiteralException(left) &&
                    !isLiteralException(right) &&
                    !isWithinSortCallback(context)) {
                    context.report({
                        message: (0, utils_1.toEncodedMessage)(`Convert operands of this use of "${operator}" to number type.`, [left, right]),
                        loc: context
                            .getSourceCode()
