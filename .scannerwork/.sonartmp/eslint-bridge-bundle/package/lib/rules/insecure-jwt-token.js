...
exports.rule = {
    ...
    create(context) {
        ...
        function checkCallToSign(callExpression, thirdArgumentValue, secondaryLocations) {
            const unsafeAlgorithmProperty = (0, utils_1.getPropertyWithValue)(context, thirdArgumentValue, 'algorithm', 'none');
            if (unsafeAlgorithmProperty) {
                const unsafeAlgorithmValue = (0, utils_1.getValueOfExpression)(context, unsafeAlgorithmProperty.value, 'Literal');
        ...
        function checkCallToVerify(callExpression, publicKey, thirdArgumentValue, secondaryLocations) {
            const algorithmsProperty = (0, utils_1.getObjectExpressionProperty)(thirdArgumentValue, 'algorithms');
            if (!algorithmsProperty) {
                if ((0, utils_1.isNullLiteral)(publicKey)) {
                    raiseIssueOn(callExpression.callee, VERIFY_MESSAGE, secondaryLocations);
                }
                return;
            }
            const algorithmsValue = (0, utils_1.getValueOfExpression)(context, algorithmsProperty.value, 'ArrayExpression');
            if (!algorithmsValue) {
                return;
            }
            const algorithmsContainNone = algorithmsValue.elements.some(e => {
                const value = (0, utils_1.getValueOfExpression)(context, e, 'Literal');
        ...
        function raiseIssueOn(node, message, secondaryLocations) {
            context.report({
                node,
                message: (0, utils_1.toEncodedMessage)(message, secondaryLocations, Array(secondaryLocations.length).fill(SECONDARY_MESSAGE)),
            });
        }
        return {
            CallExpression: (node) => {
                const callExpression = node;
                const isCallToSign = (0, utils_1.isCallToFQN)(context, callExpression, 'jsonwebtoken', 'sign');
                const isCallToVerify = (0, utils_1.isCallToFQN)(context, callExpression, 'jsonwebtoken', 'verify');
                ...
                const thirdArgumentValue = (0, utils_1.getValueOfExpression)(context, thirdArgument, 'ObjectExpression');
