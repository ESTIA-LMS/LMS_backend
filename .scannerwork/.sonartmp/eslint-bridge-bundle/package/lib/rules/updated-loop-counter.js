...
exports.rule = {
    ...
    create(context) {
        ...
        function checkCounter(counter, block) {
            const variable = (0, utils_1.getVariableFromName)(context, counter.name);
            if (!variable) {
                return;
            }
            variable.references.forEach(ref => {
                if (ref.isWrite() && isUsedInsideBody(ref.identifier, block)) {
                    context.report({
                        node: ref.identifier,
                        message: (0, utils_1.toEncodedMessage)(`Remove this assignment of "${counter.name}".`, [counter], ['Counter variable update']),
                    });
                }
            });
        }
...
function collectCountersForX(updateExpression, counters) {
    if (updateExpression.type === 'VariableDeclaration') {
        ...
        (0, utils_1.resolveIdentifiers)(updateExpression, true).forEach(id => counters.push(id));
    }
}
