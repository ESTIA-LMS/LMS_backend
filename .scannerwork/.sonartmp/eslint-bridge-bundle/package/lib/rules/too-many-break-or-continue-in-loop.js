    ...
    create(context) {
        ...
        function leaveScopeAndCheckNumberOfJumps(node) {
            ...
            if (jumps && jumps.length > 1) {
                ...
                context.report({
                    loc: firstToken.loc,
                    message: (0, utils_1.toEncodedMessage)('Reduce the total number of "break" and "continue" statements in this loop to use one at most.', jumps, jumps.map(jmp => jmp.type === 'BreakStatement' ? '"break" statement.' : '"continue" statement.')),
                });
            }
        }
        return {
